#!/bin/sh
# preremove-controller.sh - stop and disable the controller service, but
# only on a genuine removal -- never on an upgrade, where the old package's
# preremove script runs *before* the new package's files are unpacked.
# Stopping the unit here would leave a monitoring service down until an
# operator noticed and manually restarted it after the upgrade.
#
